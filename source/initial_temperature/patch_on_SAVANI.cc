...
#include <aspect/initial_temperature/patch_on_SAVANI.h>
...
namespace aspect
{
  ...
  {
    template <int dim>
    PatchOnSAVANI<dim>::PatchOnSAVANI ()
    {}


    template <int dim>
    void
	PatchOnSAVANI<dim>::initialize ()
    {
    ...
    initial_temperature (const Point<dim> &position) const
    {
      ...
      //add smoothing between the two models
      else if (depth > max_grid_depth - smoothing_length_scale && depth < max_grid_depth)
        {
          ...
          vs_perturbation = savani.get_Vs(position)*(scale_factor) + ascii_grid_vs(position)*(1.0-scale_factor);
        }
      else
        {
          vs_perturbation = savani.get_Vs(position);
        }
      ...
      const double background_temperature = this->get_material_model().is_compressible() ?
                                            ...
                                            savani.reference_temperature;

      // get the Vs to density conversion
      double vs_to_density = 0.0;
      if (savani.vs_to_density_method == savani.file)
        vs_to_density = savani.profile.get_data_component(Point<1>(depth), savani.vs_to_density_index);
      else if (savani.vs_to_density_method == savani.constant)
        vs_to_density = savani.vs_to_density_constant;
      else
      ...
      if (depth > no_perturbation_depth_patch)
        ...
        temperature_perturbation =  -1./savani.thermal_alpha * density_perturbation;
      else
    ...
    template <int dim>
    void
    PatchOnSAVANI<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection ("Initial temperature model");
      {
        prm.enter_subsection ("Patch on SAVANI");
        {
          prm.declare_entry ("Maximum grid depth", "700000.0",
                             Patterns::Double (0),
                             "The maximum depth of the Vs ascii grid. The model will read in  "
                             "Vs from SAVANI below this depth.");
          prm.declare_entry ("Smoothing length scale", "200000.0",
                             ...
                             "values from the ascii grid and SAVANI at each point in the region of smoothing.");
          prm.declare_entry ("Remove temperature heterogeneity down to specified depth", boost::lexical_cast<std::string>(-std::numeric_limits<double>::max()),
                             Patterns::Double (),
                             "This will set the heterogeneity prescribed by the Vs ascii grid and SAVANI to zero "
          ...
          Utilities::AsciiDataBase<dim>::declare_parameters(prm,
                                                            "$ASPECT_SOURCE_DIR/data/initial-temperature/patch-on-SAVANI/test/",
                                                            "upper_shell_3d.txt");
    ...
    template <int dim>
    void
    PatchOnSAVANI<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Initial temperature model");
      {
        prm.enter_subsection ("Patch on SAVANI");
      ...
      prm.leave_subsection ();
      savani.initialize_simulator (this->get_simulator());

      // Note: parse_parameters will call initialize for us
      savani.parse_parameters(prm);
...
namespace aspect
{
  namespace InitialTemperature
  {
    ASPECT_REGISTER_INITIAL_TEMPERATURE_MODEL(PatchOnSAVANI,
                                              "patch on SAVANI",
                                              ...
                                              "combined with SAVANI. Note the required format of the "
                                              ...
                                              "See SAVANI documentation for details on input parameters in the "
                                              "SAVANI perturbation subsection. "
